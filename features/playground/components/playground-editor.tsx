...
export const PlaygroundEditor = ({
  ...
  // Store command id (string|null). addCommand is not disposable
  ...
  const createInlineCompletionProvider = useCallback(
    (monaco: Monaco) => {
      return {
        provideInlineCompletions: async (_model: any, position: any, _context: any, _token: any) => {
  ...
  const handleEditorDidMount: OnMount = (editor, monaco) => {
    ...
    // addCommand returns a command id (string|null); it is not disposable
  ...
  useEffect(() => {
    return () => {
      ...
      // No dispose for addCommand id
      tabCommandRef.current = null
    }
  }, [])

  // Sanitize and type the Monaco options to satisfy the editor prop
  const monacoOptions: MonacoEditor.IStandaloneEditorConstructionOptions = {
    ...(defaultEditorOptions as any),
    lineNumbers: ((defaultEditorOptions as any).lineNumbers ?? "on") as
      | "on"
      | "off"
      | "relative"
      | "interval"
      | ((lineNumber: number) => string),
    minimap: defaultEditorOptions.minimap
      ? {
          ...(defaultEditorOptions.minimap as any),
          size: ((defaultEditorOptions as any).minimap?.size ?? "fit") as "proportional" | "fill" | "fit",
          showSlider: ((defaultEditorOptions as any).minimap?.showSlider ?? "mouseover") as "always" | "mouseover",
        }
      : { enabled: true, size: "fit", showSlider: "mouseover" },
  }
